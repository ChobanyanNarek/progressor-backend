...
- [Architecture](#architecture)
  ...
  - [Core Concepts](#core-concepts)
    ...
    - [Database Layer](#database-layer)
      ...
      - [UUID v4 Primary Keys](#uuid-v4-primary-keys)
...
`@Transactional()` (from `typeorm-transactional`) is **available** for wrapping
multiple writes in a single atomic unit. It is not a blanket pattern — apply it
only where a single logical operation performs **two or more** dependent writes
that must succeed or fail together. Single-write handlers do not need it (TypeORM
already wraps each `save`/`update` in its own transaction).

```typescript
@Transactional()
async upsertDetails(/* ... */): Promise<void> {
  // Multiple dependent writes wrapped in one transaction.
}
```

> In the current codebase the only consumer is
> `upsert-memory-point-details.handler.ts`, which writes a memory point and its
> child details atomically.

#### UUID v4 Primary Keys
All primary keys are **UUID v4**, generated by the database via TypeORM's
`@PrimaryGeneratedColumn('uuid')` on `AbstractEntity`. See
[ADR-0010](adr/0010-uuid-v4-primary-keys.md) for the full rationale (and why
v7 is recorded there as a possible future option).
...
Because v4 IDs are random, do **not** rely on the primary key for chronological
ordering. Sort by the dedicated `createdAt` column instead:

```typescript
// Chronological ordering via the createdAt timestamp column
await this.userRepository.find({ order: { createdAt: 'ASC' } });
```
